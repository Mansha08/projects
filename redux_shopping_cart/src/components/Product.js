...
const Product = ({ post }) => {
  ...
  return (
    ...
    hover:scale-110 transition duration-300 ease-in gap-3 p-4 m-10 rounded-xl outline "
    >
      <div className="">
      ...
      <div className="flex justify-between gap-12 items-cente w-full mt-5 ">
        <div>
          <p className="text-green-600 font-semibold">${post.price}</p>
        </div>
        {cart.some((p) => p.id == post.id) ? ( //in cart present an those id who similar as current id
          //remove button
